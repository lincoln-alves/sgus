...
BEGIN
	...
	DECLARE @NiveisExternos TABLE (ID_NivelOcupacional INT) INSERT INTO @NiveisExternos SELECT ID_NivelOcupacional FROM	FN_ObterNiveisOcupacionaisExternos()
	...
	SELECT
		ISNULL(MAX(N.NM_Nome), '') AS NomeNivelOcupacional,
	...
	WHERE
		MO.ID_NivelOcupacional IN (SELECT ID_NivelOcupacional FROM @NiveisExternos)
		AND (@IdUf IS NULL OR MO.ID_UF = @IdUf)
		AND CAST (MT.DT_Termino AS DATE) BETWEEN @DataInicio
		AND @DataFim
